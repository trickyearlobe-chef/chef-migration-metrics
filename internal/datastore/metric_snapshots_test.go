...
func TestInsertMetricSnapshot_MissingOrganisationID(t *testing.T) {
    db := &DB{}
    ...
    if got := err.Error(); got != "datastore: organisation ID is required to insert a metric snapshot" {
...
func TestMetricSnapshot_ZeroValue(t *testing.T) {
    ...
    if ms.ID != "" {
        t.Errorf("zero-value ID should be empty, got %q", ms.ID)
    }
    if ms.SnapshotType != "" {
...
func TestInsertMetricSnapshot_ValidationOrder(t *testing.T) {
    ...
    // Organisation ID present, rest missing — should fail on snapshot type.
    _, err = db.insertMetricSnapshot(context.TODO(), nil, InsertMetricSnapshotParams{
        OrganisationID: "org-1",
    })
    if err == nil {
    ...
    // Organisation ID + snapshot type present, data missing — should fail on data.
    _, err = db.insertMetricSnapshot(context.TODO(), nil, InsertMetricSnapshotParams{
        OrganisationID: "org-1",
        SnapshotType:   "readiness_summary",
    })
