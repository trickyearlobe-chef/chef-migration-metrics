...
func minimalRerunAllTestKitchenStore() *mockStore {
	return &mockStore{
		DeleteAllGitRepoTestKitchenResultsFn: noopDeleteFn(),
	}
}

func TestHandleAdminRerunAllTestKitchen_MethodNotAllowed(t *testing.T) {
	r := newTestRouterWithTrigger(minimalRerunAllTestKitchenStore(), nil)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/rerun-all-test-kitchen", nil)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
...
func TestHandleAdminRerunAllTestKitchen_HappyPath_WithTrigger(t *testing.T) {
	var calls atomic.Int32
	store := minimalRerunAllTestKitchenStore()
	r := newTestRouterWithTrigger(store, succeedingTrigger(&calls))

	req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/rerun-all-test-kitchen", nil)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
...
func TestHandleAdminRerunAllTestKitchen_HappyPath_WithoutTrigger(t *testing.T) {
	store := minimalRerunAllTestKitchenStore()
	r := newTestRouterWithTrigger(store, nil)
...
func TestHandleAdminRerunAllTestKitchen_DeleteError(t *testing.T) {
	store := &mockStore{
...
func minimalGitRepoRescanTestKitchenStore() *mockStore {
	return &mockStore{
		ListGitReposByNameFn: func(ctx context.Context, name string) ([]datastore.GitRepo, error) {
			return []datastore.GitRepo{
				{Name: name},
		...
		DeleteGitRepoTestKitchenResultsByRepoFn: noopDeleteByRepoFn(),
	}
}

func TestHandleGitRepoRescanTestKitchen_MethodNotAllowed(t *testing.T) {
	r := newTestRouterWithTrigger(minimalGitRepoRescanTestKitchenStore(), nil)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/git-repos/my-repo/rescan-test-kitchen", nil)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
...
func TestHandleGitRepoRescanTestKitchen_HappyPath_WithTrigger(t *testing.T) {
	var calls atomic.Int32
	store := minimalGitRepoRescanTestKitchenStore()
	r := newTestRouterWithTrigger(store, succeedingTrigger(&calls))

	req := httptest.NewRequest(http.MethodPost, "/api/v1/git-repos/my-repo/rescan-test-kitchen", nil)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
...
func TestHandleGitRepoRescanTestKitchen_HappyPath_WithoutTrigger(t *testing.T) {
	store := minimalGitRepoRescanTestKitchenStore()
	r := newTestRouterWithTrigger(store, nil)
...
func TestHandleGitRepoRescanTestKitchen_NotFound(t *testing.T) {
	store := &mockStore{
		ListGitReposByNameFn: func(ctx context.Context, name string) ([]datastore.GitRepo, error) {
			return nil, nil
		},
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/git-repos/nonexistent/rescan-test-kitchen", nil)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
