...
// ---------------------------------------------------------------------------
// Cookbook-node usage params building logic
// ---------------------------------------------------------------------------

// buildUsageParams replicates the Step 8 linkage logic from collectOrganisation
// so it can be tested in isolation without a database.
func buildUsageParams(
	nodeCookbookVersions map[string]map[string]string,
	snapshotIDMap map[string]string,
	cookbookIDMap map[string]map[string]string,
) (params []datastore.InsertCookbookNodeUsageParams, missingCookbooks int) {
	...
	return params, missingCookbooks
}

func TestBuildUsageParams_AllMatch(t *testing.T) {
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "5.0.1", "openssl": "8.0.0"},
		"db-01":  {"mysql": "10.0.0"},
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
	...
	cookbookIDMap := map[string]map[string]string{
		"apache2": {"5.0.1": "cb-111"},
		"openssl": {"8.0.0": "cb-222"},
		"mysql":   {"10.0.0": "cb-333"},
	}

	params, missing := buildUsageParams(nodeCookbookVersions, snapshotIDMap, cookbookIDMap)

	if missing != 0 {
		t.Errorf("expected 0 missing, got %d", missing)
	}
	// 2 cookbooks on web-01 + 1 on db-01 = 3 records.
	if len(params) != 3 {
		t.Fatalf("expected 3 usage params, got %d", len(params))
	}
	...
	for i, p := range params {
		...
		if p.CookbookVersion == "" {
			t.Errorf("param[%d]: cookbook version is empty", i)
		}
	}

	// Build a set of (snapshotID, cookbookID) pairs for easy lookup.
	...
	if !seen[pair{"snap-aaa", "cb-222"}] {
		t.Error("missing usage record for web-01 ↔ openssl")
	}
	if !seen[pair{"snap-bbb", "cb-333"}] {
		t.Error("missing usage record for db-01 ↔ mysql")
	}
}

func TestBuildUsageParams_MissingCookbookName(t *testing.T) {
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "5.0.1", "unknown_cb": "1.0.0"},
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
	}
	...
	if len(params) != 1 {
		t.Errorf("expected 1 usage param, got %d", len(params))
	}
}

func TestBuildUsageParams_MissingCookbookVersion(t *testing.T) {
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "99.0.0"}, // version doesn't exist in the map
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
	}
	cookbookIDMap := map[string]map[string]string{
		"apache2": {"5.0.1": "cb-111"}, // only version 5.0.1 exists
	}
	...
	if len(params) != 0 {
		t.Errorf("expected 0 usage params, got %d", len(params))
	}
}

func TestBuildUsageParams_MissingSnapshotID(t *testing.T) {
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "5.0.1"},
		"ghost":  {"apache2": "5.0.1"}, // no snapshot ID for this node
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
		// "ghost" is not in the map
	}
	cookbookIDMap := map[string]map[string]string{
		"apache2": {"5.0.1": "cb-111"},
	}
	...
	// Missing snapshot ID is silently skipped, not counted as a missing cookbook.
	if missing != 0 {
		t.Errorf("expected 0 missing (snapshot skip is silent), got %d", missing)
	}
	if len(params) != 1 {
		t.Errorf("expected 1 usage param, got %d", len(params))
	}
...
func TestBuildUsageParams_EmptyInputs(t *testing.T) {
	params, missing := buildUsageParams(nil, nil, nil)
	if missing != 0 {
		t.Errorf("expected 0 missing, got %d", missing)
	}
	if len(params) != 0 {
...
func TestBuildUsageParams_EmptyNodeCookbooks(t *testing.T) {
	// nodeCookbookVersions is populated but all nodes have empty cookbook maps
	// (this shouldn't happen in practice since we filter on len > 0, but
	// test the edge case).
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {},
	}
...
func TestBuildUsageParams_MultipleMissing(t *testing.T) {
	// Node runs 3 cookbooks, 2 of which are not in the ID map.
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "5.0.1", "missing1": "1.0.0", "missing2": "2.0.0"},
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
	}
...
func TestBuildUsageParams_SameCookbookDifferentNodes(t *testing.T) {
	// Two nodes running the same cookbook at the same version.
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "5.0.1"},
		"web-02": {"apache2": "5.0.1"},
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
		"web-02": "snap-bbb",
	...
	if len(params) != 2 {
		t.Fatalf("expected 2 usage params, got %d", len(params))
	}
	...
	for _, p := range params {
		...
		snapshotIDs[p.NodeSnapshotID] = true
	}
	if !snapshotIDs["snap-aaa"] {
		t.Error("missing usage record for snap-aaa")
	}
	if !snapshotIDs["snap-bbb"] {
		t.Error("missing usage record for snap-bbb")
	}
}

func TestBuildUsageParams_SameCookbookDifferentVersions(t *testing.T) {
	// Two nodes running the same cookbook at different versions.
	nodeCookbookVersions := map[string]map[string]string{
		"web-01": {"apache2": "5.0.1"},
		"web-02": {"apache2": "6.0.0"},
	}
	snapshotIDMap := map[string]string{
		"web-01": "snap-aaa",
		"web-02": "snap-bbb",
	...
	cookbookIDMap := map[string]map[string]string{
		"apache2": {"5.0.1": "cb-111", "6.0.0": "cb-222"},
	}
	...
	// Verify different cookbook IDs for different versions.
	type pair struct{ snap, cb string }
	seen := make(map[pair]bool)
	for _, p := range params {
		seen[pair{p.NodeSnapshotID, p.ServerCookbookID}] = true
	...
	if !seen[pair{"snap-bbb", "cb-222"}] {
		t.Error("missing usage record for web-02 ↔ apache2@6.0.0")
	}
}
