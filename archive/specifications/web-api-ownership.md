...
| `/api/v1/ownership/merge` | POST | Fold one owner into another, moving its aliases too | admin |
| `/api/v1/ownership/duplicates` | GET | List owner pairs that may be the same person | viewer+ |
| `/api/v1/ownership/duplicates/rescan` | POST | Rebuild that list (runs detached; returns 202) | operator+ |
