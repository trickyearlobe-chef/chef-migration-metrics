...
Test Kitchen and its kitchen drivers are **provided by Chef Workstation** installed on the host. They are **not** bundled with the application — there is no embedded Ruby runtime. The application resolves the `kitchen` binary from `PATH`.

External prerequisites depend on the configured driver: vCenter API access for `vcenter`, Proxmox API access for `proxmox`, etc. See [Test Kitchen Driver Abstraction](test-kitchen-drivers.md) for the supported driver list and requirements.
...
3. **Generate environment overlay** — Create a temporary `.kitchen.local.yml` file in the cookbook directory that overrides the Chef Client version provisioner attribute. The overlay generation is driver-aware. At minimum the overlay overrides the provisioner so the target Chef Client version is installed:
   ...
   For the supported hypervisor drivers (`vcenter`, `proxmox`), the overlay additionally replaces the entire `driver:` block, remaps platforms using the platform image map, and injects credentials from the secret store. See [Test Kitchen Driver Abstraction](test-kitchen-drivers.md) for the full overlay specification including credential injection, platform mapping, and driver-specific settings.

   The overlay contents are determined by inspecting the cookbook's `.kitchen.yml` to identify the driver in use and the configured target driver.
...
- If Test Kitchen is not available on `PATH` (Chef Workstation not installed), Test Kitchen testing is disabled at startup (see Startup Validation). CookStyle-only analysis continues.
