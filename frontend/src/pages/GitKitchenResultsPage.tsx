...
const STATUS_OPTIONS = ["all", "passed", "failed", "timed out", "errored", "pending"] as const;
...
function SummaryCards({ results }: { results: GitKitchenResult[] }) {
  const counts = { total: results.length, passed: 0, failed: 0, pending: 0, "timed out": 0, errored: 0 };
  ...
  const cards: { label: string; value: number; color: string }[] = [
    ...
    { label: "Timed Out", value: counts["timed out"], color: "text-yellow-700" },
...
function ResultsTable({ results }: { results: GitKitchenResult[] }) {
  if (results.length === 0) {
    return <p className="text-sm text-gray-500">No results match the current filters.</p>;
  }
  return (
      ...
      <table className="min-w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-100">
          {results.map((r) => {
            ...
            const colorCls = RESULT_STATUS_COLORS[status] || "bg-gray-100 text-gray-500";
            return (
              <tr key={r.id} className="hover:bg-gray-50">
                <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-800">
                  {r.git_repo_name}
                </td>
                <td className="whitespace-nowrap px-4 py-2 text-gray-600">{r.platform_name}</td>
                <td className="whitespace-nowrap px-4 py-2 text-gray-600">{r.suite_name}</td>
                <td className="whitespace-nowrap px-4 py-2 text-gray-600">{r.target_chef_version}</td>
...
export function GitKitchenResultsPage() {
  ...
  const filtered = results.filter((r) => {
    if (cookbookFilter && !r.git_repo_name.toLowerCase().includes(cookbookFilter.toLowerCase()))
      return false;
    if (platformFilter && !r.platform_name.toLowerCase().includes(platformFilter.toLowerCase()))
      return false;
    if (statusFilter !== "all" && resultStatus(r) !== statusFilter) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Per-instance Test Kitchen results across all batches.</p>
      </div>

      {/* Filter bar */}
      <div className="flex flex-wrap items-end gap-3">
        <div className="min-w-[180px] flex-1">
          <label className="mb-1 block text-xs font-medium text-gray-600">Cookbook</label>
          <input
            type="text"
