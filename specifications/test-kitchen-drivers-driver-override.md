...
2. **Platform image mapping** — a per-platform lookup from the kitchen platform name to a driver-specific image identifier (template name/ID, AMI ID, etc.).
...
| Profile | Status | Driver Gem | Image Field | Typical Secrets |
|---------|--------|-----------|-------------|-----------------|
| `vcenter` | Supported (production) | kitchen-vcenter | `template` | `vcenter_password` |
| `proxmox` | Supported (PoC) | kitchen-proxmox | `template_id` | `proxmox_token_secret` |
| `vra` | Planned (UI placeholder, not wired) | kitchen-vra | `password` | `password` |
| `ec2` | Planned (UI placeholder, not wired) | kitchen-ec2 | `ami` | `aws_secret_access_key` |
| `vagrant` | Planned (UI placeholder, not wired) | kitchen-vagrant | `box` | None |

Only `vcenter` and `proxmox` are wired to a hypervisor backend. The `vra`, `ec2`, and `vagrant` profiles exist as overlay-profile stubs and UI dropdown placeholders; selecting them does not yet provision a target. The driver gems are provided by Chef Workstation on the CMM host.
