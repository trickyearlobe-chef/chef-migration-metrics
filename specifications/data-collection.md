...
- **Origin reconciliation (preferred-base migration)**: the self-heal above only fires when the old remote is *dead*. During a slow estate migration (e.g. Stash → GitLab) the old remote often stays *reachable*, so a fetch would keep succeeding against the old base and the clone would never follow the repo to its new home. To make migration fully automatic, before reusing an existing clone the collector ranks the clone's on-disk `origin` against the ordered `git_base_urls`. If any base ranks **above** the current origin (a preferred base, including the case where the origin's base was removed from config entirely), the collector probes each higher-priority candidate with a cheap `git ls-remote`. The first candidate that exists becomes the new origin (`git remote set-url origin`), and the pull below fetches from it; the stale `git_repos` row for the old URL is removed by the existing cross-URL cleanup. If no preferred candidate has the repo yet, the clone stays on its current origin and fetches as before. This is self-limiting: a repo already on the top-priority base has nothing ranked above it and is never probed. No config flag day and no manual `reset-git` are required — each cookbook migrates the moment it appears at a preferred base.
...
| Read origin URL | `git remote get-url origin` | Emits the configured origin URL. Used to rank an existing clone against `git_base_urls` for origin reconciliation. |
| Probe candidate exists | `git ls-remote <URL>` | Refs-only reachability check — no working tree, no object download. Exit 0 means a repo exists at the URL. Any error is treated as "not present" (fall back to current origin). |
| Re-point origin | `git remote set-url origin <URL>` | Migrates an existing clone to a preferred base without deleting local objects; the next `fetch`/`reset` pulls from the new base. |
