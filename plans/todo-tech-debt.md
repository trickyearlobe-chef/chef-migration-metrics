...
- [ ] **Listen rebind not wired for active auto-443 or ACME.** `serverctl.Controller` is adopted for plain-`off` mode and for healthy static TLS where a single HTTPS listener owns the configured port (`https443Ln == nil`, with an optional explicit `http_redirect_port` redirect listener — H4b-2). When the auto-443 lifeboat is active (`https443Ln != nil`) the configured port is only a redirect source and HTTPS lives on a separate 443 listener; ACME additionally owns a port-80 challenge/redirect listener. Both need a topology-aware re-plan (HTTPS + redirect/challenge) that **H4b-3 / H4c** own. Until then a `listen_address`/`port`/`tls` change in those modes reports `restart_required` (the no-rebinder fallback) and applies on the next restart. **Strategic fix:** land the H4b-3 auto-443 re-plan and the H4c ACME rebuild, adopting a topology-aware controller for those modes.
...
- [ ] **A live static rebuild serves a single HTTPS listener — no auto-443 lifeboat re-plan.** A live off→static or static field change binds HTTPS on the configured port (plus an optional explicit `http_redirect_port` redirect — H4b-2); the boot-time auto-443 (tls.md § 1.5, HTTPS on a separate 443 with the configured port as a redirect) is not re-planned on a live transition, and the controller is not adopted at boot when auto-443 is active (`https443Ln != nil`). **Strategic fix:** H4b-3 — topology-aware auto-443 re-plan + adopt.
