...
- [ ] **CookStyle work is logged under two scopes, so collection logs are cluttered** — server cookbook scanning reports under `collection_run` while git repo scanning reports under `cookstyle_scan`. The split is not deliberate: the collector scopes *everything* it emits as `collection_run` (`collector.go:773`) and hands that logger to the server cookbook pipeline, so per-cookbook download and scan progress lands there; the git path's per-repo lines come from the scanner in `internal/analysis`, which scopes them `cookstyle_scan` (`cookstyle.go:390,517,659,996,1060`). So the boundary follows which *layer* emits the line, not which source is being scanned. **Effect:** filtering the log to `collection_run` to follow a collection is drowned in per-cookbook scan chatter, and filtering to `cookstyle_scan` shows only half the scanning. **Strategic fix:** pass a `ScopeCookstyleScan` logger into the server cookbook pipeline for per-item download/scan progress, leaving run-level summaries under `collection_run`. Both sources then report consistently and the collection log stays readable. Noted 2026-07-30 while watching a fleet-wide rescan.
