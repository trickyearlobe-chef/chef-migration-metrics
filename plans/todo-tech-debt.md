...
- [ ] **Coarse status-column migration backfills repeated three times** — migrations 0041 (`cookstyle_status` ← `passed`), 0042 (node readiness `status` ← `is_ready`), and the 0043 fingerprint backfill each approximate the new column from a boolean and rely on "self-heals on next scan", because SQL can't re-derive `needs_review` (needs offences + classification). Correct but imprecise until the next scan. **Strategic fix:** write one reusable first-boot Go re-derivation backfill that recomputes status from stored offences/verdicts for all rows, and reuse it rather than accepting imprecise SQL per migration. Added 2026-06-26 (`feature/cookstyle-violations-browser`). **Queued for its own branch.**
