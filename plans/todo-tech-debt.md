...
- [ ] `DataStore` interface has 211 methods (`webapi/store.go`, measured 2026-08-02; up from 138 — the cookstyle status/classification/fingerprint cluster on `feature/cookstyle-violations-browser` added ~10 + ~450 lines of `store_mock_test.go`) — split into domain-specific sub-interfaces (nodes, cookbooks, kitchen, auth, config, **cookstyle**) composed into the full interface. The cookstyle methods are the cleanest first slice to extract (`CookstyleStore`) since they were all added together. **Queued for its own branch after the cookstyle branch merges.**
...
- [ ] **Legacy server/TLS environment overrides duplicate config-store settings** — 16 of the 19 `CHEF_MIGRATION_METRICS_*` lookups in `internal/config` set server port and TLS/ACME values. They are not needed for lockout recovery: startup already degrades to a self-signed certificate and then to plain HTTP rather than failing, so the in-code fallback ladder is the anti-lockout mechanism. What remains is a second way to set values that belong in the config store, which is how a file/env model keeps getting reintroduced — including into the specs. **Fix:** confirm nothing in packaging or the customer deployment depends on them, then remove, leaving only the values that unlock the database (`DATABASE_URL`, `CMM_CREDENTIAL_ENCRYPTION_KEY`, its rotation partner, and `CHEF_MIGRATION_METRICS_CONFIG`). Noted 2026-07-31.
...
- [ ] **Export job rows expire on the *logging* retention period** — `collector.go` purges `export_jobs` rows using `cfg.Logging.RetentionDays`. The comment says this is deliberate, but it means changing log retention silently changes how long export history is kept, and the export purge is still tied to the tail of a collection run (log retention no longer is). **Fix:** give exports their own `exports.retention_days` and move the purge onto the export cleanup ticker, which already exists.
- [ ] **`PurgeLogEntriesOlderThanDays` has no production caller** — the retention ticker calls `PurgeLogEntriesBefore` directly, so the days-based wrapper is now reachable only from its own test. Remove it, or keep it deliberately as the public API and have the ticker use it.
- [ ] **Two identical btree indexes on `log_entries.timestamp`** — `idx_log_entries_retention` and `idx_log_entries_timestamp` are the same index under different names, doubling write cost and disk on a table measured at 26GB in production. Drop one.
- [ ] **`collection_runs.checkpoint_start` is dead** — a column, a params-struct field, an UPDATE binding and three scan sites, all carrying a value nothing ever sets. Its only production read is interpolating 0 into a log message. It exists because page-level checkpointing was specified and never built. **Fix:** drop the column and its plumbing (needs a migration), or build the feature. Leaving it makes the next reader believe resume is incremental — it is not.
- [ ] **An interrupted collection re-collects the whole organisation** — resume abandons the run and re-queues the org from the beginning. At 62k nodes per org an interruption late in a run discards that org's entire collection. Not a bug, but an unpriced cost: either build page-level resume or decide the cost is acceptable.
- [ ] **Fingerprint `correctable` is write-only but hashed** — computed, stored and included in the content hash, yet re-derivation resolves classification from cop name and severity alone. Consequence: changing the field's meaning changes every hash and appends a spurious "changed" fingerprint fleet-wide. **Fix:** read it, or drop it from the projection and the hash.
