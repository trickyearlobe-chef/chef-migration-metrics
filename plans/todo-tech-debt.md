...
## TLS — ACME Hostname Self-Registration Re-asserts on Renewal, Not on Config Change

Recorded 2026-06-10 (TLS-in-DB Chunk 9a, Route 53 hostname self-registration).

- [ ] **Hostname A-record self-registration does not react immediately to config changes.** `setupACME` (`cmd/.../acme.go`) snapshots the ACME config and wires the `HostnameRegistrar` into the `Renewer` via `WithHostnameRegistrar`, so the A record is UPSERTed at startup and at the start of every renewal cycle (which corrects a changed DHCP IP). But `tls-acme.md § 3.13` also lists "when relevant configuration changes" as a re-assert trigger, and that path is **not** wired: toggling `register_hostname`/`hostname_ip`/`hostname_interface`/`hostname_ttl`/`domains` via the UI does not take effect until the next renewal cycle (default ~12h) or a restart. This is consistent with the rest of ACME config, which is snapshot-at-startup (same root cause as the backup-scheduler item below). **Strategic fix:** have ACME mode subscribe to config-store change notifications and rebuild/re-register on change (ideally a shared "ACME config changed" reload path that also re-evaluates domains and the challenge solver). Until then, the renewal-cycle re-assert plus a restart cover the operational cases.
- [ ] **Hostname registration failures are logged but not surfaced in TLS status.** `tls-acme.md § 3.13` says a registration failure should be "surfaced in TLS status"; Chunk 9a logs an `ERROR` on the `tls` scope (fail-soft) but does not add a field to `TLSStatus` (`internal/webapi/handle_server_tls_status.go`) — that holder only models listener/cert health (degraded kind), and a hostname failure must not flip the cert into degraded. **Strategic fix:** add a dedicated ACME/hostname status surface (planned for the Chunk 10 ACME status panel) so the operator sees the last hostname-registration error without conflating it with cert degradation.
