...
- [ ] **Spec/code drift: `cop-classification.md` Re-eval step 2 names columns that don't exist.** The spec says "update `status`/`passed` + `complexity_score` in `*_cookstyle_results`." Actual schema: the cookstyle-results tables hold only `passed` (no `status`/`complexity_score` column); complexity lives in separate `server_cookbook_complexity`/`git_repo_complexity` tables. The propagator updates `passed` in the result tables and complexity in the complexity tables — intent met, spec wording stale. **Fix (needs owner sign-off):** reword the spec to match the materialisation split, or add the columns if a single-table SoT is wanted.
