...
- [ ] **off→static via H4a serves a single HTTPS listener — no auto-443 lifeboat or `http_redirect_port` topology.** A live off→static binds HTTPS on the configured port only; the boot-time auto-443 (tls.md § 1.5) and any `http_redirect_port` redirect listener are not re-planned on a live transition. `applyServerListener` refuses static targets with `http_redirect_port != 0` (→ restart_required). **Strategic fix:** H4b — full listener-topology rebuild (HTTPS + redirect/443).
- [ ] **Same-mode static topology changes are not applied live (H4b).** A change to `min_version` / mTLS `ca_path` / `http_redirect_port` while staying in static mode (no listen or mode change) reports `process` (restart_required). The rebuild path already consumes these fields, so they only need a trigger + the topology rebuild.
