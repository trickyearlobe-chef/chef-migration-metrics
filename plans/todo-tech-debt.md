...
- [ ] **`handleCookbookRemediation` (543 lines) and `handleGitRepoRemediation` (433 lines) are single-function god-handlers** (`internal/webapi/handle_cookbook_remediation.go:33`, `handle_git_repo_remediation.go:38`). In both, the file essentially *is* the function — 543 of 576 lines, and 433 of 471. These are a different shape of debt from the cookstyle god-handler: that one held 42 declarations across five REST resources and split cleanly per resource, whereas each of these is **one** resource and one enormous function, so a per-resource split does not apply. **`handleCookbookRemediation` still carries an orphaned git-repo fallback** (verified `@ ccad467`). If no server cookbook matches, it falls back to git repos and runs the **whole git-repo pipeline inline** (`handle_cookbook_remediation.go:140` onward — `ListGitReposByName`, `GetGitRepoCookstyleResult`, `ListGitRepoComplexitiesByRepo`, `GetGitRepoAutocorrectPreview`). Structurally, all 26 of `handleGitRepoRemediation`'s distinct calls also appear in the cookbook handler; the cookbook handler adds only its four `ServerCookbook*` queries. It is a second, independent copy of the git pipeline.

**It is dead code.** This is a leftover from when server and git cop analysis shared one table and one page. The Cop Analysis Server/Git tab separation gave git repos their own page (`GitRepoRemediationPage`) and endpoint (`/api/v1/git-repos/:name/:version/remediation` → `handleGitRepoRemediation`), and the old fallback was never removed. Every UI entry point into `/cookbooks/:name/:version/remediation` now carries a **server** cookbook: `CopAnalysisTab.tsx:588` (Server tab only — the Git tab links to `/git-repos/…` at line 634), `RemediationPage.tsx:579` (priority list, `ListServerCookbooks*`, server-only), and `CookbookDetailPage.tsx:332` (Cookbooks page, `FROM server_cookbooks`; `git_repos` appears in `cookbook_filter.go` only in a `tk` CTE decorating rows with `tk_status`, not as a row source).

**Strategic fix:** delete the git fallback from `handleCookbookRemediation` and let it 404 when no server cookbook matches — by construction there cannot be one. That removes ~200 of its 543 lines and eliminates the duplication and the drift risk outright, leaving `handleGitRepoRemediation` as the single git path. **Check first:** this changes behaviour for any *direct API* caller hitting `/api/v1/cookbooks/{git-only-name}/…/remediation`; no UI path does. Found 2026-07-14 via structural comparison of the two handlers' call sets.
