...
- [ ] **off→static (and adopted single-listener static) serves a single HTTPS listener — no auto-443 lifeboat or `http_redirect_port` topology.** A live off→static binds HTTPS on the configured port only; the boot-time auto-443 (tls.md § 1.5) and any `http_redirect_port` redirect listener are not re-planned on a live transition. `applyServerListener` refuses static targets with `http_redirect_port != 0` (→ restart_required), and the controller is not adopted at boot when auto-443 is active. **Strategic fix:** H4b-2/H4b-3 — full listener-topology rebuild (HTTPS + redirect/443) and a topology-aware adopt.
- [ ] **Same-mode static `http_redirect_port` change is not applied live (H4b-2).** Adding/removing/changing the redirect listener while staying in static mode reports `process` (restart_required). H4b-1 landed live apply of the single-listener static field changes (`min_version` / mTLS `ca_path` / cert source-or-paths) via the TLS topology fingerprint, but `http_redirect_port != 0` is still refused (the old process holds the redirect port during the drain window — needs the multi-listener rebuild).
