...
- [ ] **`handleCookbookRemediation` (543 lines) and `handleGitRepoRemediation` (433 lines) are single-function god-handlers** (`internal/webapi/handle_cookbook_remediation.go:33`, `handle_git_repo_remediation.go:38`). In both, the file essentially *is* the function — 543 of 576 lines, and 433 of 471. These are a different shape of debt from the cookstyle god-handler: that one held 42 declarations across five REST resources and split cleanly per resource, whereas each of these is **one** resource and one enormous function, so a per-resource split does not apply. **The git-repo remediation pipeline exists twice** (verified `@ ccad467`, superseding an earlier line-overlap guess). `handleCookbookRemediation` branches on source: if a server cookbook exists it uses the `ServerCookbook*` queries, otherwise it falls back to git repos and runs the **whole git-repo pipeline inline** (`handle_cookbook_remediation.go:140` onward — `ListGitReposByName`, `GetGitRepoCookstyleResult`, `ListGitRepoComplexitiesByRepo`, `GetGitRepoAutocorrectPreview`). `handleGitRepoRemediation` is a second, independent implementation of that same pipeline: comparing the two functions' call sets structurally, all 26 of the git-repo handler's distinct calls also appear in the cookbook handler, which adds only its four `ServerCookbook*` queries. So the two views derive git-repo remediation by separate code paths and can drift — the same cross-view-mismatch risk this codebase has hit before.

**Strategic fix:** extract one source-agnostic remediation pipeline parameterised by source (server cookbook vs git repo) and have both handlers call it, rather than splitting each god-function in place. That removes the duplication and the drift risk together. Found 2026-07-14 via structural search over call sequences.
